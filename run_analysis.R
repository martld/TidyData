...
# Apply friendly column names for feature columns. I remove parentheses and replace abbreviations
# with longer names so that the content of the columns are better understood.
...
# The final data set has 180 rows (30 subjects with 6 activities each).
# The final data set has 21 columns (1 each for subject, activityID and activity description 
# and the 18 selected features that use mean and standard deviation.

write.table(alldata_summary, "tidy_data.txt", row.names=FALSE,sep='\t')

# Read the tidy data set back into R
